...
def generate_loader_scripts(data, output_directory, yaml_script_filename):
    ...
    with open(script_filepath, 'w') as script_file:
        ...
        script_file.write(f"export PATH=$PATH:/opt/cassandra/apache-cassandra/tools/bin/\n\n")
...
def main():
    ...
    scylla_group_values = ['CassandraSeed', 'CassandraNonSeed']
    ...
    for region_name in config['regions']:
        ...
        loader_instances = get_nodes_by_tag_and_cluster(session, cluster_name, 'Type', 'CassandraLoader', None, region_name)
        
        # Get Scylla instances in the current region, filtered by Group tag
        scylla_instances = get_nodes_by_tag_and_cluster(session, cluster_name, 'Type', 'Cassandra', scylla_group_values, region_name)
